...
def fused_cross_entropy_forward(
    ...
    with torch.cuda.device(logits.device.index):
        fused_cross_entropy_fwd_kernel[(n_rows, n_splits)](
            losses,  # data ptrs
            lse,
            z_losses,
            logits,
...
def fused_cross_entropy(
    ...
    losses, z_losses = FusedCrossEntropyLossFunction.apply(
        input,
        target,
        label_smoothing,
        logit_scale,
    ...
    if reduction == "mean":
        loss = losses.sum() / (target != ignore_index).sum()
    elif reduction == "sum":
        loss = losses.sum()
    else:
        loss = losses.view(target_shape)

    if not return_z_loss:
        return loss

    if reduction == "mean":
        z_loss = z_losses.sum() / (target != ignore_index).sum()
    elif reduction == "sum":
        z_loss = z_losses.sum()
    else:
        z_loss = z_losses.view(target_shape)
